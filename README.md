...
Dataset: A 2-year historical dataset containing daily stock prices for 10 unique symbols (ticker) in a single CSV file. I correctly obtained the directed dataset from the given link.
...
Output files will be generated in `output/result_{SYMBOL}.csv` for each of the 10 tickers.
...
├── data/
│   └── stock_data.csv      # Input: 2 years of daily stock data
└── output/
    └── result_*.csv        # Output: 10 monthly summary files
```

## Assumptions

Below, I shall be mentioning the practical assumptions I have made while solving the problem that I am required to explain.
...
Each `result_{SYMBOL}.csv` contains 24 rows with columns:
- `date`: Month-end date
